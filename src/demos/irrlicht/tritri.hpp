...
int coplanar_tri_tri(double N[3],
					 double V0[3],
					 double V1[3],
					 double V2[3],
					 double U0[3],
...
int compute_intervals_isectline(double VERT0[3],
								double VERT1[3],
								double VERT2[3],
								double VV0,
								double VV1,
...
#define COMPUTE_INTERVALS_ISECTLINE(VERT0, VERT1, VERT2, VV0, VV1, VV2, D0, D1, D2, D0D1, D0D2, isect0,     \
									isect1, isectpoint0, isectpoint1)                                       \
	if (D0D1 > 0.0f) {                                                                                      \
		/* here we know that D0D2<=0.0 */                                                                   \
		/* that is D0, D1 are on the same side, D2 on the other or on the plane */                          \
		isect2(VERT2, VERT0, VERT1, VV2, VV0, VV1, D2, D0, D1, &isect0, &isect1, isectpoint0, isectpoint1); \
...
int tri_tri_intersect_with_isectline(double V0[3],
	...
	/* compute interval for triangle 1 */
	*coplanar = compute_intervals_isectline(V0, V1, V2, vp0, vp1, vp2, dv0, dv1, dv2, dv0dv1, dv0dv2,
											&isect1[0], &isect1[1], isectpointA1, isectpointA2);
	if (*coplanar)
		return coplanar_tri_tri(N1, V0, V1, V2, U0, U1, U2);

	/* compute interval for triangle 2 */
	compute_intervals_isectline(U0, U1, U2, up0, up1, up2, du0, du1, du2, du0du1, du0du2, &isect2[0],
								&isect2[1], isectpointB1, isectpointB2);
